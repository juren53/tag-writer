...
- Listing image files in directories
"""
...
def read_metadata(file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # ExifTool returns a list of objects, we want the first one
        if metadata_list and len(metadata_list) > 0:
            # Flatten the metadata to a single level dictionary
            metadata = metadata_list[0]
            flattened = {}
            
            for key, value in metadata.items():
                # Skip SourceFile and empty values
                if key == "SourceFile" or not value:
                    continue
                
                # Store in flattened dictionary
                flattened[key] = value
                
            logger.info(f"Read {len(flattened)} metadata fields from {file_path}")
            return flattened
        else:
